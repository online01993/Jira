...
CustomField insightCF = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(10501);       
...
def objects = iqlFacade.findObjectsByIQLAndSchema(9, "objectType = Офисы and object HAVING inboundReferences(objectType = Сотрудники and \"" + "JIRA User\" = " + valueCF.getValue(issue) + ")");
if (!objects.isEmpty()) {
   MutableIssue mi = (MutableIssue) issue;
    ...
    objects = iqlFacade.findObjectsByIQLAndSchema(9, "objectType = Офисы and object HAVING inboundReferences(objectType = Сотрудники and \"" + "Почтовый адрес\" = " + valueCF.getValue(issue) + ")");
	if (!objects.isEmpty()) {
		MutableIssue mi = (MutableIssue) issue;
		mi.setCustomFieldValue(insightCF, objects);
		ComponentAccessor.getIssueManager().updateIssue(currentUser, mi, EventDispatchOption.DO_NOT_DISPATCH, false);
