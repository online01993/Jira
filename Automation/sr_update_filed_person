...
CustomField insightCF = ComponentAccessor.getCustomFieldManager().getCustomFieldObject(10300);       
...
def objects = iqlFacade.findObjectsByIQLAndSchema(9, "objectType = Сотрудники and \"" + "JIRA User\" = " + valueCF.getValue(issue));
if (!objects.isEmpty()) {
   MutableIssue mi = (MutableIssue) issue;
    ...
    objects = iqlFacade.findObjectsByIQLAndSchema(9, "objectType = Сотрудники and \"" + "Почтовый адрес\" = " + valueCF.getValue(issue));
	if (!objects.isEmpty()) {
		MutableIssue mi = (MutableIssue) issue;
		mi.setCustomFieldValue(insightCF, objects);
		ComponentAccessor.getIssueManager().updateIssue(currentUser, mi, EventDispatchOption.DO_NOT_DISPATCH, false);
